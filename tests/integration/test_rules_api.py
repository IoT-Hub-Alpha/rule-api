...
def test_rule_crud_flow(client):
    ...
    enable_response = client.post(f"/api/v1/rules/{rule_id}/enable")
    assert enable_response.status_code == 200
    assert enable_response.json()["data"]["is_enabled"] is True

    disable_response = client.post(f"/api/v1/rules/{rule_id}/disable")
    ...
    get_after_delete = client.get(f"/api/v1/rules/{rule_id}/")
